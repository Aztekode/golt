		...
		middleware := func(middlewareCall goja.FunctionCall) goja.Value {
			...
			logRequest := func() {
				...
				case status >= 200 && status < 300:
					statusBadge = fmt.Sprintf("\033[42;37;1m %3d %s", status, reset)
				case status >= 300 && status < 400:
					statusBadge = fmt.Sprintf("\033[46;37;1m %3d %s", status, reset)
				case status >= 400 && status < 500:
					statusBadge = fmt.Sprintf("\033[43;30;1m %3d %s", status, reset)
				case status >= 500:
					statusBadge = fmt.Sprintf("\033[41;37;1m %3d %s", status, reset)
				default:
					statusBadge = fmt.Sprintf("\033[100;37;1m %3d %s", status, reset)
				}
			...
			if ok {
				nextResult, err := nextFn(goja.Undefined())
				if err != nil {
					panic(err)
				}

				if attachThenable(vm, nextResult, logRequest, func(goja.Value) {
					logRequest()
				}) {
					return nextResult
				}
